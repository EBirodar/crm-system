...
class ClientHelper
{
    ...
    {
        return [
            Client::STATUS_ACTIVE => Yii::t('models', 'Актив'),
            Client::STATUS_INACTIVE => Yii::t('models', 'Неактив'),
        ];
    }
...
//    {
//        $groupList = Group::find()
//            ->select('group.id, group.name, sp.group_list AS group_list')
//            ->leftJoin('group AS sp', 'sp.group_id=group.id')
//            ->active()
//            ->asArray()
//            ->all();
//
//        return ArrayHelper::map($groupList, 'id', function ($model) {
//            return $model['name'] . ' (' . nf($model['group_list']) . ')';
//        });
//    }
}